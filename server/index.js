...
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'culaccino2026';
...
function requireAdmin(req, res, next) {
  if (verifyToken(req.cookies.cu_admin)) return next();
...
app.post('/api/admin/logout', (req, res) => {
  res.clearCookie('cu_admin');
  res.json({ ok: true });
});

app.get('/api/admin/me', (req, res) => res.json({ ok: verifyToken(req.cookies.cu_admin) }));
...
app.get('/api/admin/export.csv', requireAdmin, (req, res) => {
  ...
  res.setHeader('Content-Disposition', `attachment; filename="culaccino-bookings-${T.fmtDate(new Date())}.csv"`);
  res.send(csv);
});
