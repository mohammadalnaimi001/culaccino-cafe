/* ==========================================================
   Culaccino — front-end application
   ========================================================== */
(function () {
  'use strict';
  ...
  function renderOccasions() {
    ...
    $$('#occList .chip').forEach((b) => b.addEventListener('click', () => {
      ...
      renderOccasions();
      if (state.occasion === 'study' && state.area !== 'upper') {
        const el = $('#areaList .area[data-area="upper"]');
        if (el) el.click();
      }
    }));
  }
  ...
  function syncShisha() {
    ...
    // إذا ما في ولا قسم بيقدّم أرجيلة، الخيار بيختفي بالكامل
    if (state.cfg && !state.cfg.areas.some((x) => x.shisha)) {
      field.style.display = 'none';
      state.shisha = false;
      $('#bkShisha').checked = false;
      return;
    }
    field.style.display = '';
    const a = areaObj(state.area);
  ...
  /* أقسام اختيارية — بتنخفي حسب إعدادات server/config.js بدون أي تغيير بالتصميم */
  function initSections() {
    if (state.cfg.cafe.showShishaSection === false) {
      const sec = document.getElementById('shisha');
      if (sec) sec.style.display = 'none';
      $$('a[href="#shisha"]').forEach((a) => { a.style.display = 'none'; });
    }
  }
  ...
  function initLinks() {
    ...
    // إذا خلّيت orderUrl فاضي في server/config.js، أزرار الطلب بتختفي من الموقع
    ['#orderBtn', '#orderInfo'].forEach((s) => {
      const el = $(s);
      if (!el) return;
      if (c.orderUrl) el.href = c.orderUrl;
      else el.style.display = 'none';
    });
    const footOrder = $('#footOrder');
    if (footOrder) { if (c.orderUrl) footOrder.href = c.orderUrl; else footOrder.style.display = 'none'; }
